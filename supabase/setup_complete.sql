...
-- Execute este script no SQL Editor do Supabase após limpar o banco com cleanup_database.sql
...
-- =============================================
-- 7. DADOS INICIAIS (seed_initial_data.sql)
-- =============================================
...
INSERT INTO config.products (category_id, name, slug, description, unit, has_minimum_price, minimum_price, display_order)
SELECT 
  c.id,
  p.name,
  p.slug,
  ...
  p.minimum_price,
  p.display_order
FROM config.product_categories c
CROSS JOIN (VALUES
  ('ACM', 'acm', 'Alumínio Composto', 'm²', true, 120.00, 1),
  ('Acrílico', 'acrilico', 'Placas de acrílico', 'm²', true, 80.00, 2),
  ('Estrutura Metálica', 'estrutura-metalica', 'Estruturas metálicas para fixação', 'kg', true, 8.00, 3)
) AS p(name, slug, description, unit, has_minimum_price, minimum_price, display_order)
WHERE c.slug = 'fachada'
ON CONFLICT (slug) DO NOTHING;
...
FROM config.product_categories c
CROSS JOIN (VALUES
  ('LED', 'led', 'Iluminação LED para letreiros', 'm', true, 15.00, 1),
  ('Caixa de Luz', 'caixa-de-luz', 'Caixas de luz para sinalização', 'm²', true, 200.00, 2),
  ('Neon Flex', 'neon-flex', 'Fitas de LED Neon Flex', 'm', true, 25.00, 3)
) AS p(name, slug, description, unit, has_minimum_price, minimum_price, display_order)
WHERE c.slug = 'luminoso'
ON CONFLICT (slug) DO NOTHING;
...
FROM config.product_categories c
CROSS JOIN (VALUES
  ('Plotagem', 'plotagem', 'Impressão e plotagem de adesivos', 'm²', true, 35.00, 1),
  ('Instalação', 'instalacao', 'Serviços de instalação', 'unid', true, 150.00, 2),
  ('Projeto', 'projeto', 'Desenvolvimento de projetos', 'unid', true, 300.00, 3)
) AS p(name, slug, description, unit, has_minimum_price, minimum_price, display_order)
WHERE c.slug = 'diversos'
ON CONFLICT (slug) DO NOTHING;

-- Inserir opções de produtos para ACM
WITH product_ids AS (
  SELECT id FROM config.products WHERE slug = 'acm'
)
INSERT INTO config.product_options (product_id, name, description, unit, display_order)
SELECT 
  p.id,
  o.name,
  o.description,
...
FROM product_ids p
CROSS JOIN (VALUES
  ('ACM 3mm', 'ACM espessura 3mm', 'm²', 1),
  ('ACM 4mm', 'ACM espessura 4mm', 'm²', 2),
  ('ACM 6mm', 'ACM espessura 6mm', 'm²', 3)
) AS o(name, description, unit, display_order)
ON CONFLICT (product_id, name) DO NOTHING;

-- Inserir opções de produtos para Acrílico
WITH product_ids AS (
  SELECT id FROM config.products WHERE slug = 'acrilico'
)
INSERT INTO config.product_options (product_id, name, description, unit, display_order)
SELECT 
  p.id,
...
FROM product_ids p
CROSS JOIN (VALUES
  ('Acrílico 3mm', 'Acrílico cristal 3mm', 'm²', 1),
  ('Acrílico 5mm', 'Acrílico cristal 5mm', 'm²', 2),
  ('Acrílico 8mm', 'Acrílico cristal 8mm', 'm²', 3),
  ('Acrílico Colorido', 'Acrílico colorido diversas cores', 'm²', 4)
) AS o(name, description, unit, display_order)
ON CONFLICT (product_id, name) DO NOTHING;

-- Inserir preços base para as opções de ACM
WITH option_ids AS (
...
SELECT 
  o.id,
  CASE 
    WHEN o.name = 'ACM 3mm' THEN 120.00
    WHEN o.name = 'ACM 4mm' THEN 140.00
    WHEN o.name = 'ACM 6mm' THEN 180.00
  END
FROM option_ids o
ON CONFLICT (product_option_id) DO NOTHING;

-- Inserir preços base para as opções de Acrílico
WITH option_ids AS (
  SELECT po.id, po.name 
  FROM config.product_options po
  JOIN config.products p ON p.id = po.product_id
...
SELECT 
  o.id,
  CASE 
    WHEN o.name = 'Acrílico 3mm' THEN 80.00
    WHEN o.name = 'Acrílico 5mm' THEN 100.00
    WHEN o.name = 'Acrílico 8mm' THEN 130.00
    WHEN o.name = 'Acrílico Colorido' THEN 95.00
  END
FROM option_ids o
ON CONFLICT (product_option_id) DO NOTHING;
